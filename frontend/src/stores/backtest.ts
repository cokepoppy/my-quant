...
export const useBacktestStore = defineStore('backtest', {
  ...
  getters: {
    ...
    // 获取正在运行的回测
    runningBacktests: (state) => state.backtests.filter(b => b.status === 'running'),
    
    // 获取已完成的回测
    completedBacktests: (state) => state.backtests.filter(b => b.status === 'completed'),
    
    // 获取失败的回测
    failedBacktests: (state) => state.backtests.filter(b => b.status === 'failed'),
    ...
    bestPerformingBacktests: (state) => {
      return state.backtests
        ...
        .sort((a, b) => (b.results?.totalReturn || 0) - (a.results?.totalReturn || 0))
        .slice(0, 5)
    },
  ...
  actions: {
    ...
    // 获取回测日志
    async fetchBacktestLogs(id: string, options?: { level?: string; limit?: number }) {
      try {
        this.setLoading(true)
