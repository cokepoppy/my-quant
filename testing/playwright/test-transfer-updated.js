...
(async () => {
    ...
    try {
        ...
        console.log(`📊 当前余额: ${currentBalance.join(', ')}`);
        
        // 4. 点击刷新按钮
        console.log('🔄 步骤4: 点击刷新按钮...');
        ...
        if (refreshClicked) {
            ...
            // 5. 检查更新后的余额
            console.log('💰 步骤5: 检查更新后的余额...');
            
            const updatedBalance = await checkBalance();
            console.log(`📊 更新后余额: ${updatedBalance.join(', ')}`);
            ...
            const beforeTotal = getBalanceValue(currentBalance[0]);
            const afterTotal = getBalanceValue(updatedBalance[0]);
            const beforeAvailable = getBalanceValue(currentBalance[1]);
            const afterAvailable = getBalanceValue(updatedBalance[1]);
            ...
            const testResult = {
                ...
                beforeRefresh: currentBalance,
                afterRefresh: updatedBalance,
